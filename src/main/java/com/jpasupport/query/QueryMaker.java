...
import com.jpasupport.annotations.EntityFieldMapping;
import com.jpasupport.annotations.FKEntityFieldMapping;
import com.jpasupport.annotations.LinkEntityFieldMapping;
import com.jpasupport.constants.DBConstants;
import com.jpasupport.dto.ListViewFieldDTO;
import com.jpasupport.dto.QueryOrderByDTO;
import com.jpasupport.dto.RequestParamDTO;
import com.jpasupport.utils.CustomReflectionUtils;
import com.jpasupport.utils.DateUtils;
import com.jpasupport.utils.ParameterVerifier;
import com.jpasupport.utils.Utils;
...
public class QueryMaker<T> {
	...
	public QueryMaker(RequestParamDTO reqParamDto, Class<T> rtnCls) {
		...
		this.root = CustomReflectionUtils.getRootEntity(rtnCls);
		this.fkEntityFieldMap = CustomReflectionUtils.getFKEntityFieldMappingsMap(rtnCls);
		this.linkEntityFieldMap = CustomReflectionUtils.getLinkEntityFieldMappingsMap(rtnCls);
	}
	...
	private String getFieldClause(ListViewFieldDTO fieldDto) {
		...
		try {
			if(fieldDto.getValue() != null && !fieldDto.getValue().toString().trim().isEmpty()) {
				...
				Class cls = CustomReflectionUtils.isFieldNameExist(root, fieldDto.getDataField()) ? root : null;
				if(cls == null) {
					EntityFieldMapping fkFieldMappingValue = CustomReflectionUtils.getFKFieldMappingValueAnnotation(rtnCls, fieldName, EntityFieldMapping.class);
				...
				if(cls != null) {
					if(DBConstants.FIELD_TYPE_TEXT.equalsIgnoreCase(fieldDto.getType())) {
						...
						CustomReflectionUtils.castValue(cls, fieldDto);
					}
				}			
			}
...
//			e.printStackTrace();
	...
	private String processFieldName(String entityRef, String fieldName) {
	...
	public List<T> convertData(List list) {
					...
					for(Object obj: (Object[])listObj) {
						if(obj != null) {
							if(count == 1 ) {
								...
								CustomReflectionUtils.setFieldValue(obj, t);
							}
						}
						count++;
					}
